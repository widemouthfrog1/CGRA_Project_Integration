...
int main(){
    ...
    Shader simpleShader = Shader("/home/dylan/Documents/CGRA350/Project/Shaders/basicVertexShader.glsl", "/home/dylan/Documents/CGRA350/Project/Shaders/basicFragmentShader.glsl");
...
void generateTerrain(){
    ...
    for(int i = 0; i < terrainSize; ++i){
        for(int j = 0; j < terrainSize; ++j){
            ...
            float currentHeight = depthMap[i][j] < flatHeight ? depthMap[i][j] : depthMap[i][j] * depthMap[i][j] * heightMultiplier;

            depthMap[i][j] = currentHeight;
    ...
    for(int i = 0; i < terrainSize; ++i){
        for(int j = 0; j < terrainSize; ++j){
            depthMap[i][j] = (depthMap[i][j] - meshHeightExtremes.y) / (meshHeightExtremes.x - meshHeightExtremes.y);
            if(depthMap[i][j] > 1) std::cout << "Failed" << std::endl;
        }
    }
...
void depositSediment(int xPos, int zPos, float cellOffsetX, float cellOffsetZ, float amountOfSediment){
    ...
    //std::cout << depthMap[xPos][zPos] << ", " << depthMap[xPos][zPos + 1] << ", " << depthMap[xPos + 1][zPos] << ", " << depthMap[xPos + 1][zPos + 1] << std::endl;
    ...
    //std::cout << depthMap[xPos][zPos] << ", " << depthMap[xPos][zPos + 1] << ", " << depthMap[xPos + 1][zPos] << ", " << depthMap[xPos + 1][zPos + 1] << std::endl << std::endl;
}
...
void initializeBrushIndices(int brushRadius){
    ...
    std::cout << "Size of erosionVectors: " << erosionBrushIndices.size() << ", " << erosionBrushIndices[0].size() << std::endl;
}
...
glm::vec3 calculateGradientAndHeight(float xPos, float zPos){
    ...
    //std::cout << "Initial Height: " << heightNW << ", " << heightNE << ", " << heightSW << ", " << heightSE << std::endl;
    ...
    if(pointHeight < 0 || pointHeight > 1){ std::cout << "The height is fucked" << std::endl; }
    //std::cout << "Gradient and Height: " << xGradient << ", " << yGradient << ", " << pointHeight << std::endl << std::endl;
...
void erodeTerrain(){
    ...
    float inertiaConstant = 0.05;
    float minSedimentCapacity = 0.01;
    float sedimentCapacityFactor = 4.00;
    float depositSpeed = 0.5;
    float erodeSpeed = 0.000005;
    ...
    for(int i = 0; i < numIterations; i++) {
        ...
        for (int lifetime = 0; lifetime < maxDropletLifetime; lifetime++) {
            ...
            if ((dirX == 0 && dirY == 0) || posX < 0 || posX >= terrainSize - 1 || posY < 0 || posY >= terrainSize - 1) {
                
                std::cout << "Stopped simulating" << std::endl;
                break;
            }
            ...
            if (sediment > sedimentCapacity || deltaHeight > 0) {
                ...
                speed = sqrt(speed * speed + deltaHeight * gravityConstant);
...
void placeTrees(){
    ...
    std::cout << "Done placing trees" << std::endl;
}
